...
def gap_finder(op, lo, cl, hi, lookback=90, entryZscore=1, is_down_gap=True):
    """Descrption:
...
if __name__ == "__main__":
    ...
    down_gap = gap_finder(op=op, lo=lo, cl=cl, hi=hi, lookback=lookback, 
                          entryZscore=entryZscore, is_down_gap=True)
    ...
    pos_long = over_ma * down_gap
    pos_short = under_ma * up_gap

    for index, row in pos_long.iterrows():
        pass
    print(type(pos_long))
    print(pos_long.shape)
