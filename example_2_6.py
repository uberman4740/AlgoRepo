...
if __name__ == "__main__":
    ...
    modelo2 = pd.ols(y=y, x=x, window_type='rolling', window=100)
    #print(type(modelo2.beta))
    #print(modelo2.beta.tail(10))
    data = data[99:]
    betas = modelo2.beta

    numunits = subtract(data['EWC'], multiply(betas['x'], data['EWA']))
   
    #print(type(numunits))
    #print(numunits.head(5))
    #print(numunits.tail(5))
    fig1=numunits.plot()
    plt.title('EWC-hedgeRatio*EWA', fontdict=font)   
    #plt.xlabel('Dates', fontdict=font)
    #plt.ylabel('Beta', fontdict=font)     
